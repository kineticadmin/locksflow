...
export default function AdminAvailability() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080808', color: '#F2EDE5' }}>
      ...
      <div style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '0 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: 64 }}>
          ...
          <div style={{ display: 'flex', gap: 32, alignItems: 'center' }}>
            {NAV_ITEMS.map(item => (
              <a key={item.href} href={item.href} style={{ color: item.href === '/admin/availability' ? '#F97316' : 'rgba(242,237,229,0.5)', fontSize: 11, textDecoration: 'none', fontFamily: 'var(--font-unbounded)', letterSpacing: 1 }}>
                {item.label}
              </a>
            ))}
      ...
      <div style={{ padding: '32px', display: 'grid', gridTemplateColumns: '1fr 380px', gap: 32, alignItems: 'start' }}>

        {/* Colonne gauche — planning hebdo */}
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <div>
              <h2 style={{ fontFamily: 'var(--font-unbounded)', fontSize: 18, fontWeight: 900, marginBottom: 4 }}>Planning hebdo</h2>
              <p style={{ color: '#888', fontSize: 12 }}>Active les jours et les créneaux disponibles.</p>
            </div>
            <button
              onClick={saveConfig}
              disabled={saving}
              style={{ background: saved ? '#10B981' : '#F97316', color: '#080808', border: 'none', padding: '10px 24px', borderRadius: 6, fontFamily: 'var(--font-unbounded)', fontSize: 11, fontWeight: 900, cursor: saving ? 'not-allowed' : 'pointer', letterSpacing: 1, transition: 'background 0.3s' }}
            >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {config.map(day => (
              <div key={day.day_of_week} style={{ background: day.active ? 'rgba(249,115,22,0.05)' : 'rgba(255,255,255,0.02)', border: `1px solid ${day.active ? 'rgba(249,115,22,0.2)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 12, padding: '16px 20px', transition: '0.2s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: day.active ? 16 : 0 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-unbounded)', fontSize: 13, fontWeight: 700, color: day.active ? '#F2EDE5' : '#555', width: 90 }}>
